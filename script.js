...
function move(dx, dy) {
    console.log('Move called:', dx, dy, 'Game running:', gameRunning, 'Paused:', isPaused); // Debug log

    if (!gameRunning || isPaused) {
        console.log('Move blocked - game not running or paused'); // Debug log
        return false;
    }

    if (!currentPiece) {
        console.log('Move blocked - no current piece'); // Debug log
        return false;
    }
    ...
    if (!collision(currentPiece.shape, newX, newY)) {
        currentX = newX;
        currentY = newY;
        console.log('Move successful to:', newX, newY); // Debug log
        return true;
    }

    if (dy > 0) {
        console.log('Piece collision, placing piece'); // Debug log
        placePiece();
    }
...
function rotate() {
    console.log('Rotate called - Game running:', gameRunning, 'Paused:', isPaused); // Debug log

    if (!gameRunning || isPaused) {
        console.log('Rotate blocked - game not running or paused'); // Debug log
        return;
    }

    if (!currentPiece) {
        console.log('Rotate blocked - no current piece'); // Debug log
        return;
    }
    ...
    if (!collision(rotated, currentX, currentY)) {
        ...
        console.log('Piece rotated successfully'); // Debug log
    } else {
        console.log('Rotation blocked by collision'); // Debug log
    }
}
...
// Mobile controls with proper touch handling
function setupMobileControls() {
    ...
    function addTouchAndClickEvent(element, callback) {
        if (!element) {
            console.log('Element not found for touch event'); // Debug log
            return;
        }

        console.log('Setting up touch events for:', element.id); // Debug log
        ...
        element.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('Click event triggered on:', element.id); // Debug log
            addVisualFeedback();
        ...
        element.addEventListener('touchstart', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('Touch start event triggered on:', element.id); // Debug log
            addVisualFeedback();
            triggerHapticFeedback();
            callback();
        }, { passive: false });
        ...
        element.addEventListener('pointerdown', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('Pointer down event triggered on:', element.id); // Debug log
            addVisualFeedback();
            triggerHapticFeedback();
            callback();
        });
    ...
    // Setup each button
    addTouchAndClickEvent(leftBtn, () => {
        console.log('Left button pressed'); // Debug log
        move(-1, 0);
    });

    addTouchAndClickEvent(rightBtn, () => {
        console.log('Right button pressed'); // Debug log
        move(1, 0);
    });

    addTouchAndClickEvent(downBtn, () => {
        console.log('Down button pressed'); // Debug log
        if (move(0, 1)) {
            score += 1;
            updateScore();
        }
    ...
    addTouchAndClickEvent(rotateBtn, () => {
        console.log('Rotate button pressed'); // Debug log
        rotate();
    });

    addTouchAndClickEvent(dropBtn, () => {
        console.log('Drop button pressed'); // Debug log
        hardDrop();
    });

    // Also setup reset and pause buttons for mobile
    const resetBtn = document.getElementById('resetBtn');
    const pauseBtn = document.getElementById('pauseBtn');

    addTouchAndClickEvent(resetBtn, () => {
        console.log('Reset button pressed'); // Debug log
        init();
    });

    addTouchAndClickEvent(pauseBtn, () => {
...
// Add debug info for mobile detection
window.addEventListener('DOMContentLoaded', () => {
    console.log('Screen width:', window.innerWidth);
    console.log('Screen height:', window.innerHeight);
    console.log('User agent:', navigator.userAgent);

    // Test if mobile controls are visible
    const mobileControls = document.querySelector('.mobile-controls');
    if (mobileControls) {
        const styles = window.getComputedStyle(mobileControls);
        console.log('Mobile controls display:', styles.display);
    }
});
